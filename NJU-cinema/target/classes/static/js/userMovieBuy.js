...
function addActivity(orderInfo) {
    getRequest('/activity/get/' + movieId,
        function (res) {
            var data = res.content;
            orderInfo.activities = data;
...
function addTicket(orderInfo) {

    getRequest('/schedule/' + scheduleId,
        function (res) {

            var data = res.content;
            fare = data.fare * selectedSeats.length;
            for (i = 0; i < selectedSeats.length; i++) {
                orderInfo.ticketVOList[i] = {
                    "id": order.ticketId[i], "userId:": sessionStorage.getItem('id'),
...
function addCoupons(orderInfo) {
    getRequest("/coupon/" + sessionStorage.getItem('id') +'/'+orderInfo.total+ '/get',
        function (res) {
            var data = res.content;
            orderInfo.coupons = data;
            addActivity(orderInfo);
        },
        function (error) {
            alert(JSON.stringify(error));
        }
...
function postPayRequest() {
    ...
    $('#buyModal').modal('hide');
    var Tickecpl = new Array();
    for (i = 0; i < order.ticketId.length; i++) {
        Tickecpl[i] = {id: order.ticketId[i], couponId: order.couponId};
    }
    postRequest('/ticket/buy', Tickecpl,
        function (res) {
            issueCoupon();
        },
        function (error) {
...
function  postPayRequestByVip() {
    $('#order-state').css("display", "none");
    $('#success-state').css("display", "");
    $('#buyModal').modal('hide');
    loc=window.location.href;
    getRequest('/vip/' + sessionStorage.getItem('id') + '/' + parseFloat($('#order-actual-total').text().replace(" ¥" ,'')) + '/pay',
        function (res) {
        if(res.success){
            var Tickecpl = new Array();
            for (i = 0; i < order.ticketId.length; i++) {
            ...
            postRequest('/ticket/vip/buy', Tickecpl,
                function (res) {
                    console.log(Tickecpl);
                    //issueCoupon();
                },
                function (error) {
                    alert("支付失败");
...
function issueCoupon() {
    getRequest("/coupon/"+sessionStorage.getItem('id')+"/"+activities[0].coupon.id+"/issue",
        function (res) {
            //
        },
        function (error) {
...
function deleteCouponId() {
    /*废弃，留着以备不时之需*/
    getRequest("/coupon/"+sessionStorage.getItem('id')+"/"+10+"/delete",
        function (res) {
