$(document).ready(function(){
    ...
    function renderMovieList(list) {
        ...
        list.forEach(function (movie) {
            ...
            if(movie.status){
                movieOff +=
                    ...
                    "<span class='label error-bg'>" + '已下架' + "</span>" +
                    "<span class='movie-want'><i class='fa fa-heart' style='color: red'></i>" + (movie.likeCount || 0) + "人想看</span>" +
                    "</div>" +
                    ...
                    "<div class='movie-operation'><a href='/admin/movieDetail?id="+ movie.id +"' style=\"color: #007DD3;\">详情</a></div></div>" +
                    "</div>"+
                    "</li>";
            }else {
                movieOn +=
                    ...
                    "<div class='movie-title'>" +
                    "<span class='primary-text'>" + movie.name + "</span>" +
                    "<span class='label primary-bg'>" + (new Date(movie.startDate)>=new Date()?'未上映':'热映中') + "</span>" +
                    "<span class='movie-want'><i class='fa fa-heart' style='color: red'></i>" + (movie.likeCount || 0) + "人想看</span>" +
                    "</div>" +
                    "<div class='movie-description dark-text'><span>" + movie.description + "</span></div>" +
                    "<div>类型：" + movie.type + "</div>" +
        ...
        $('.movie-on-list').append(movieOn);
        $(".movie-off-list").append(movieOff)
    }
});